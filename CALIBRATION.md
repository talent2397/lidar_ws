...
> **v12 状态（2026-08-14）**：动态补偿升级为 **v3 + 地面平面慢速反馈（v4）**。
> 离线仿真：171903 翻滚转弯运动段 Δz0 从 −15.8±8.5cm 降到 −0.1±4.0cm（τ=0.5s）。
> 已实现并经离线回放接线验证，**待实机转弯 bag 复测**。
...
地面平面，把残差（z0/roll/pitch）做 **τ=0.5s 的 EMA 慢速回灌**到 TF：
...
- z 泄漏双积分保留，作为地面不可见（剧烈翻滚/遮挡）时的惯性兜底。

离线仿真结果（`scripts/analyze_ground_feedback.py`，τ=0.5s）：

| Bag | 模式 | 运动 Δz0 修正前 | 运动 Δz0 修正后 |
|---|---|---|---|
| 171903 | 翻滚转弯 | −15.8±8.5cm | −0.1±4.0cm |
| 173447 | 直线 | −3.0±6.5cm | 0.0±3.0cm |

已知限制：离线回放时 bag 内 IMU 缺帧会被补偿器“补算”放大姿态漂移（回放假象），
实机验证需用连续 IMU 数据；剧烈翻滚导致地面不可见时反馈保持，靠 IMU 兜底。
