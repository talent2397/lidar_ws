...
| 静止穿透（点<-0.2m） | ~11-12° 倾斜 | 0.06%（102121 实测） |
| 运动穿透（点<-0.2m） | 8.3%（静态 TF） | 0.24%（102121 实测） |
...
rslidar_sdk_node
  ...
  └── /rslidar_imu_data_2  (角速度+加速度, 200Hz)
         ...
         ↓
point_cloud_fusion (TF2 → world, 按到达时刻-50ms 回查 TF, 80ms 同步窗口, 拼接)
         ↓
...
## 3. 动态补偿（suspension_compensator v3）
...
- 直接订阅 `/tf` 维护 2s 环形缓冲，按“点云到达时刻 - 50ms”取最近动态变换，
  不依赖 tf2 的过去时刻查询；
- 发布后记录已融合时刻；任一侧有新点云即可重新配对，不清空缓存（避免丢配对）。
...
ros2 topic hz /merged_points          # ~4.4Hz（受 sync_window=0.08s 限制）
...
> Python 脚本（`point_cloud_fusion_py.py`、`suspension_compensator.py`）已软链到 install，
> 修改后无需重新构建；C++ 融合节点（`src/point_cloud_fusion_node.cpp`）以及 URDF/launch
> 修改后需要重新构建。
...
| 问题 | 原因 | 状态 |
|------|------|:--:|
| 两雷达帧不同步 | 无 PTP Master，扫描相位独立 | ⚠️ 80ms 同步窗口缓解（sync_window=0.08s） |
| 运动时点云残余畸变 | 0.1s 扫描周期 + 无逐点时间戳 | ⚠️ 当前 0.24%（102121 实测） |
...
## 12. FAST-LIO2 方案A（已实施，2026-08-05）

### 12.1 架构

```text
rslidar_sdk (POINT_TYPE=XYZIRT)
  ├─ /rslidar_points_1    x/y/z/intensity/ring/timestamp (主机时钟, 秒)
  └─ /rslidar_imu_data_1  200Hz, frame=rslidar_1 (加速度 g, 角速度 rad/s)

rslidar_lio_adapter
  ├─ /fastlio/lidar_points  x/y/z/intensity/time(相对秒,float32)/ring
  └─ /fastlio/imu          原样转发

spark_fast_lio (lidar_type=2 velodyne 路径)
  ├─ /odometry /path /cloud_registered*  (~60Hz odom)
  └─ TF: odom -> base_link

world_anchor: IMU 初始姿态 + odom 初始 z -> 静态 TF world -> odom
```

### 12.2 关键参数（`src/rslidar_lio_adapter/config/fastlio_airy.yaml`）

| 参数 | 值 | 说明 |
|---|---|---|
| `preprocess.lidar_type` | 2 | Velodyne 风格 PointCloud2 处理 |
| `preprocess.scan_line` | 96 | Airy 96 线 |
| `preprocess.scan_rate` | 10 | Hz |
| `preprocess.timestamp_unit` | 0 | 适配节点输出相对秒 |
| `preprocess.blind` | 0.3 | 与驱动 min_distance 0.15 匹配 |
| `blind_for_human_pilots` | 0.0 | 关闭 pilot 区剔除 |
| `mapping.extrinsic_T` | [0,0,0] | 平移初值，可在线标定 |
| `mapping.extrinsic_R` | R_lidar2imu = R(imu2lidar DIFOP q)^T | 见 12.4 |
| `mapping.fov_degree` | 360 | Airy 360°×90° |
| `point_filter_num` / `filter_size_map` | 4 / 0.2 | 可调 |
| `gravity_alignment.enable_gravity_alignment` | false | 由 world_anchor 代替 |

### 12.3 时间同步与 IMU 量纲（实测确认）

- `use_lidar_clock: false` 时，点云逐点 `timestamp` 与 IMU `header.stamp` 均为主机时钟，
  FAST-LIO 直接用消息时间戳，无需旧方案的 80ms 窗口软同步。
- 适配节点把逐点绝对秒转换为**相对首点秒**（float32 `time` 字段），
  FAST-LIO 内部转 ms（100ms/帧），避免绝对时间在 float 下的精度损失。
- IMU 角速度单位已是 rad/s；加速度单位是 g，FAST-LIO 内部按模长归一化
  （`acc * G_m_s2 / |acc|`），因此无需额外 ×9.8。
- 2026-08-05 修复：驱动 `getTimeHost()` 由 system_clock 改为
  steady_clock + 首调时刻偏移，避免 NTP 校时回跳导致点云/IMU 时间戳倒退；
  适配节点对输出 IMU 时间戳做单调递增兜底。
- 录制 bag 时不要同时录大点云话题，否则 IMU 消息会因写盘拥塞丢包（见 PROGRESS.md）。

### 12.4 外参（lidar → IMU）

DIFOP 四元数 q(imu→lidar) = (-0.701147, 0.712996, -0.00452085, -0.00301585)，
取逆得到 R_lidar2imu 写入配置：
...
平移初值 [0,0,0]（IMU 内置于雷达）；需要更高精度时
`bash start_fastlio.sh extrinsic_est:=true` 在线估计后冻结。

### 12.5 world_anchor（世界锚点）

- 启动时用 IMU 加速度计静止均值估计初始姿态（roll/pitch，无需运动）；
- 等 odom 稳定后读取初始 z 偏差并补偿；
- 发布静态 TF `world → odom`，保证地面水平且 `base_link` z≈0.345m。
- 实测：world→base_link z=0.354m，roll/pitch 约 -5.2°/4.9°（机器人实际静止姿态倾斜）。

### 12.6 实测结果（2026-08-05）

**静止验收**

| 指标 | 结果 |
|---|---|
| odom 频率 | 40-70Hz（单雷达模式实测 ~100Hz） |
| 静止 116s 漂移 | 端到端 0.27cm，最大偏差 2.15cm |
| CPU | LIO 核心 ~1 核，整链约 2 核（单雷达模式更低） |
| 地图 | /cloud_registered 正常，PCD 可保存 |

**运动验收（bag `fastlio_20260805_152810`，79s）✅**

| 指标 | 结果 | 目标 |
|---|---|---|
| 往返闭合 | 13.25 cm | <15cm 或 <2% 行程 ✅ |
| 点云断流 | 0 次（最大间隔 0.12s） | 无 >0.5s 断档 ✅ |
| 最大单步跳变 | 5.99 cm | <20cm ✅ |
| 运动段 z 波动 | -45~+27cm，停止后回到 ±4cm | 水平场地复核 |

首轮运动 bag（`fastlio_20260805_143249`）因录制时 CPU 过载（RViz + 稠密点云 + 双雷达）
出现最长 2.5s 点云断流，触发 LIO 发散（往返闭合 ~100cm）；修复为单雷达驱动 +
关闭稠密发布 + 预处理抽稀后，第二轮录制无断流、达标。

### 12.7 待验收

- ⏳ 水平地面穿透率/地面厚度（命令见 PROGRESS.md）；
- ⏳ 30min 连续运行长稳；
