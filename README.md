...
>
> **2026-08-14 回放验证**：单 LIO 在新标定 XYZIRT bag 上跑通 —— 修复 IMU
> reliable 顺序问题与 world_anchor 坐标系后，`/cloud_registered_base` 地面残差
> 1.57cm、穿透 0.17%（max 5.35% 高速帧）；平面高度 −4.6cm 固定偏移待
> `world_z_offset` 微调。下一步双 LIO。
...
- 浏览器打开 http://<机器人IP>:8899（脚本启动时会打印地址；
  同网段只需放行 8899 端口），页面里可直接切换 3D/俯视 BEV、点大小、话题开关
- 显示话题：
