...
| rslidar_1 z / rpy | 0.020 / (1.731, 3.22, 0.02) | **0.0193 / (-1.5946, 0.0033, -3.1147)** |
| rslidar_2 z / rpy | 0.020 / (-1.621, 0.06, 0.02) | **0.0532 / (-1.4142, -0.0231, 0.0238)** |
...
|------|-----|------|
| 球体半径 | 345 mm | 球心到球表面 |
| 球心离地 | 395 mm | base_link(world 坐标下) |
...
```
world (地面 z=0)
└── base_link (球心, z=0.395)          ← 静态
    ├── rslidar_1   — 左雷达 native frame ← 动态补偿 (suspension:=true)
    └── rslidar_2   — 右雷达 native frame ← 动态补偿 (suspension:=true)
```

- `world → base_link` 为静态变换 `(0, 0, 0.395)`，由 launch 中 `static_transform_publisher` 发布。
...
- z 值 = 雷达实际安装高度 − 球心离地 0.395m：
  - rslidar_1：总高 0.4143 m → z = 0.0193 m
  - rslidar_2：总高 0.4482 m → z = 0.0532 m
...
静止段实测（164735 bag）：仅 0.04% 的点 z<-0.2m；
运动段经 fusion v2 离线验证：0.75% 的点 z<-0.2m（原 4.3%）。
...
ros2 run tf2_ros tf2_echo world base_link   # Translation [0, 0, 0.395]
```
...
| 外参 z 为纯机械偏移 (v10) | 总高 = 0.395 + z，不再混入地面补偿 |
