...
> 状态：**计划待确认，尚未实施**。本文档只描述改造方案，不包含任何代码改动。
...
| 项 | 结论 |
|---|---|
| FAST-LIO 读取的字段名（time/timestamp） | 待填 |
| 时间单位/相对绝对处理 | 待填 |
| lidar_type / scan_line / scan_rate / timestamp_unit 配置名 | 待填 |
| extrinsic_T/R 格式与初值 | 待填 |
| odom/TF/map 输出 topic 与 frame 名 | 待填 |
| Airy 是否需要专用 handler | 待填 |
