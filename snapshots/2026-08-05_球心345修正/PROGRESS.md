...
> 代码快照：`snapshots/2026-08-04_1800/`（345 修正前）、`snapshots/2026-08-05_球心345修正/`（最新）。

## 一句话状态

**静态标定已完成并验证通过；动态补偿（v3）和 C++ 融合节点已实现；C++ 节点崩溃根因（rclcpp 时间源不匹配）已修复并通过自检；2026-08-05 已将球心离地高度修正为 0.345m（球半径）。等待重新启动真机验证。**
